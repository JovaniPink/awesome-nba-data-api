...
```bash
uv pip compile --python-version 3.14 --upgrade --output-file=requirements.txt --strip-extras requirements.in
uv pip compile --python-version 3.14 --upgrade --output-file=requirements-dev.txt --strip-extras requirements-dev.in
```
...
dependency PRs can refresh both source constraints and compiled artifacts. CI also recompiles both
locks without upgrades and fails if the checked-in artifacts are stale.
